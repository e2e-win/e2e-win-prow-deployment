...
function main () {
    ...
    local TEST_INFRA_REPO="http://github.com/e2e-win/test-infra"
    
    local KUBEKINS_IMAGE_DIR="images/kubekins-e2e"
    local BOOTSTRAP_IMAGE_DIR="images/bootstrap"

    set_go
    local clone_target="$GOPATH/src/k8s.io/test-infra"

    start_dockerd

    folder=$(clone "$TEST_INFRA_REPO" "$clone_target")
    pushd "$folder"
        ...
        build_target "$KUBEKINS_IMAGE_DIR" "$IS_PULL_REQUEST"
        build_target "$BOOTSTRAP_IMAGE_DIR" "$IS_PULL_REQUEST"
    popd
